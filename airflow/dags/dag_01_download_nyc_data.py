...
task_start = DummyOperator(task_id="task_start", dag=dag_01_download_nyc_data)
...
def function_download_data(
    download_date: str, month_delta: int = 0, type_tripdata: str = "yellow"
) -> None:
    """
    ...
    if type_tripdata not in ("yellow", "green"):
        raise ValueError(
            'ERROR: invalid type_tripdata parametrer, must be "yellow", "green"'
        )

    download_date = datetime.strptime(download_date, "%Y-%m-%d")
    delta = relativedelta(
        year=download_date.year, month=download_date.month, months=month_delta
    )
    ...
    url = URL_PATTERN.format(
        type_tripdata=type_tripdata, str_download_date=str_download_date
    )
    try:
        response = requests.get(url, timeout=TIMEOUT_SECONDS)
    except (
        rexcep.ConnectionError,
        rexcep.HTTPError,
        rexcep.Timeout,
        rexcep.TooManyRedirects,
        rexcep.RequestException,
    ) as exc:
        print(exc)
        return
    ...
    df["duration"] = (df["dropoff_datetime"] - df["pickup_datetime"]).apply(
